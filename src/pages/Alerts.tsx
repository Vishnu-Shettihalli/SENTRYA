    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="space-y-4">
        {alerts.length === 0 ? (
          ...
          alerts.map((alert, i) => {
            const isCommunity = alert.message.includes("COMMUNITY REPORT:");
            const displayMessage = isCommunity ? alert.message.replace("COMMUNITY REPORT:", "").trim() : alert.message;
            
            let bgClass = "bg-amber-50 text-amber-600";
            let tagClass = "bg-amber-100 text-amber-700";
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all flex items-center gap-6 group">
                <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm shrink-0", bgClass)}>
                  <AlertTriangle className="w-7 h-7" />
                </div>
                
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-lg font-black text-slate-900 truncate group-hover:text-brand-600 transition-colors">{displayMessage}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest", tagClass)}>
                      {isCommunity ? "Global Report" : `${alert.risk_level} Risk`}
                    </span>
